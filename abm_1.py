...
class Predator(Agent):
    def __init__(self, env, phi=None, x=np.random.randint(RES_X), y=np.random.randint(RES_Y), dx=0, dy=0, L=STR_LENGTH):
        super().__init__(env, phi, x, y, dx, dy)
        self.L = L
        self.neighbors = dict(predators = [], preys = [])
        self.local_readings = [(prey, prey.x, prey.y, prey.dx, prey.dy) for prey in self.neighbors["preys"]] 
    ...
    def get_neighborhood(self):
        ...
        for prey in self.env.preys:
            ...
            if np.sqrt((prey.x - self.x)**2 - (prey.y - self.y)**2) <= RADIUS:
                self.neighbors["preys"].append(prey)
                self.local_readings.append((prey, prey.x, prey.y, prey.dx, prey.dy, ))
    ...
    def phi(self):
        ...
        #process messages
        #messages are the sensorial inputs from other agents limited to some l
        self.I_ = self.I + mb.messages.values
