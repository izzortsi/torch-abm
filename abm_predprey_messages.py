...
while not quit_loop:
    ...
    if _iter % 20 == 0:
        ...
        logger.info(f"Fittest predator: {env.get_fittest_predator()}.\n")
    ...
    if _iter % 100 == 0:
        ...
        logger.info(f"Increasing communication channel's size by 1; it's now: {STR_LENGTH}.\n")

        print(f"Message board size: {env.message_board.size}.\n")
        logger.info(f"Message board size: {env.message_board.size}.\n")

        print(f"Message board messages: {env.message_board.messages}.\n")
        logger.info(f"Message board messages: {env.message_board.messages}.\n")

    if len(env.preys) == 0:
        
        print("No preys left.")
        logger.info("No preys left.")
